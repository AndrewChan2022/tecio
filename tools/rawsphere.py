...
def gen_sphere_to_np(d, h, w, radius, center, dtype=np.uint8):
    ...
    distance_array = np.zeros((d, h, w), dtype=np.uint8)
    ...
    if dtype == np.uint8:
        distance_array[mask_within_radius] = np.ceil(distance_values).astype(np.uint8)
    else:
...
def main(out, d, h, w, dtype = np.uint8):
    ...
    factor = 1
    values = gen_sphere_to_np(d, h, w, int(w/4), (int(w/2), int(h/2), int(d/2)))
    tic = time.time()
